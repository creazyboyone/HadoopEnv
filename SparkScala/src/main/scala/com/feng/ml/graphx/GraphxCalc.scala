...
object GraphxCalc {
  def main(args: Array[String]): Unit = {
    ...
    graph.triplets.foreach(
      t => println(s"triplet : ${t.srcId},${t.srcAttr},${t.dstId},${t.dstAttr},${t.attr}")
    )
    println
    println("==> 找出图中边属性大于 5 的边：")
    graph.edges.filter(e => e.attr > 5).collect.foreach(e => println(s"${e.srcId} to ${e.dstId} att ${e.attr}"))
    println
    println("==> 列出边属性大于 5 的 triplets：")
